...
* **OOP Approach:**
    ...
    class Character {
      ...
      render(position) {
        console.log(`Rendering character at ${position} with font ${this.font} and color ${this.color}`);
      }
    }
...
* **Functional Approach:**
    ...
    * **Example:**
        ...
        const vectorRenderer = (shape) => console.log(`Drawing a circle with radius ${shape.radius}`);
        const rasterRenderer = (shape) => console.log(`Drawing pixels for a circle with radius ${shape.radius}`);
    ...
    * **Example:**
        ...
        const createSubject = () => {
          ...
          return {
            ...
            notifyObservers: (data) => observers.forEach(observer => observer.update(data))
          };
        };

        const subject = createSubject();
        const observer1 = createObserver(data => console.log(`Observer 1: ${data}`));
        const observer2 = createObserver(data => console.log(`Observer 2: ${data}`));
...
* **Functional Approach:**
    ...
    * **Example:**
        ...
        const createColleague = (mediator) => {
          ...
          mediator.addColleague({ send: (msg) => mediator.send(msg, { receive }), receive });
        };
        ...
        createColleague(mediator);
        mediator.addColleague({ send: (msg) => mediator.send(msg, { receive: (msg) => console.log(`Received: ${msg}`) }), receive: (msg) => console.log(`Received: ${msg}`) });
        mediator.send('Hello, Colleague2!', { receive: (msg) => console.log(`Received: ${msg}`) });
        ```
...
* **Functional Approach:**
    ...
    * **Example:**
        ...
        const context = { setState: (state) => (context.state = state), state: stateA };
...
* **Functional Approach:**
    ...
    * **Example:**
        ...
        const createBuilder = () => {
          let product = {};
          return {
            buildPart1: () => { product.part1 = 'Part1'; },
            buildPart2: () => { product.part2 = 'Part2'; },
            getResult: () => product
          };
        };
...
// Adapter Function
const adapter = (specificRequest) => () => `Adapter: (TRANSLATED) ${specificRequest().split('').reverse().join('')}`;
...
class Composite extends Component {
  ...
  operation() {
    return `Composite: ${this.name} [${this.children.map(child => child.operation()).join(', ')}]`;
  }
}
...
const createComposite = (name) => {
  ...
  return {
    ...
    operation: () => `Composite: ${name} [${children.map(child => child.operation()).join(', ')}]`
  };
};
